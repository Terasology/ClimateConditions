...
public class ClimateMapWidget extends CoreWidget {
    ...
    public void onDraw(Canvas canvas) {
        if (climateSystem != null) {
            canvas.drawFilledRectangle(JomlUtil.rectangleiFromMinAndSize(0, 0, SIZE_OF_IMAGE, SIZE_OF_IMAGE),
                    Color.WHITE);
            for (int i = 0; i < SIZE_OF_IMAGE; i++) {
                for (int j = 0; j < SIZE_OF_IMAGE; j++) {
                    ...
                    float color = climateSystem.getClimateConditionsBase().get(playerPosition.x + offsetX, height,
